...
func insert(args []string) {
	...
	scanner = bufio.NewScanner(yamlFileHandle)
	for scanner.Scan() {
		if line == startNode.Value.LineNumber {
...
func delete(args []string) {
	...
	// bfs subtree to find max line number, this is our end of delete
	lastLine := findLastLineOfSubtree(startNode)
	...
	scanner = bufio.NewScanner(yamlFileHandle)
	for scanner.Scan() {
		if line > lastLine || line < startNode.Value.LineNumber {
			outFile.WriteString(scanner.Text() + "\n")
		}
		line++
	}
